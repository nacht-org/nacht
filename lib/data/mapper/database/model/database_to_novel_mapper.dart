...
class DatabaseToNovelMapper implements Mapper<db.Novel, NovelEntity> {
  ...
  @override
  NovelEntity from(db.Novel input) {
    return NovelEntity(
      id: input.id,
      title: input.title,
      url: input.url,
      ...
      status: statusMapper.from(input.statusId),
      lang: input.lang,
      volumes: [],
      metadata: [],
      workType: workTypeMapper.from(input.workTypeId),
      readingDirection: readingDirectionMapper.from(input.readingDirectionId),
