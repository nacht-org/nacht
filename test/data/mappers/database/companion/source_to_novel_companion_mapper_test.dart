  ...
  mapperGroup<sources.Novel, NovelsCompanion>(
    ...
    test: (mapper) {
      mapperTest<sources.Novel, NovelsCompanion>(
        ...
        stub: () {
          when(mockStatusMapper.from(sources.NovelStatus.ongoing))
              .thenReturn(StatusSeed.ongoing);
          when(mockWorkTypeMapper.from(const sources.OriginalWork()))
              .thenReturn(WorkTypeSeed.original);
          when(mockReadingDirectionMapper.from(sources.ReadingDirection.ltr))
