...
void main() {
  ...
  group('DatabaseToNovelMapper', () {
    mapperTest(
      ...
      stub: () {
        when(mockStatusMapper.from(StatusSeed.completed))
            .thenReturn(sources.NovelStatus.completed);
        when(mockWorkTypeMapper.from(WorkTypeSeed.translationMtl))
            .thenReturn(const sources.TranslatedWork.mtl());
        when(mockReadingDirectionMapper.from(ReadingDirectionSeed.ltr))
